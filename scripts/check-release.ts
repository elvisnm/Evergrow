...
try {
  ...
  if(current.some(e=>{const [major,minor]=e.version.split('.').map(Number);return major!==0||minor>6;}))throw new Error('Prototype versions must stay within the approved 0.6 milestone until the next milestone is explicitly approved.');
