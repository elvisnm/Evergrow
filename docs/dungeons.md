...
The minimap shows the local explored floor. The large map supports drag/zoom, discovered chest/boss hover labels and an Overworld button. Floor discovery never writes to the surface chart. Surface entrance tooltips distinguish active expeditions, **Reward waiting** after boss victory, and **Claimed** only after the final chest is fully delivered. Chamber altar labels and map markers likewise wait for their own chest claim; finished waves or a partial delivery do not mark the activity complete. Boss markers may still say **Defeated** to describe the enemy itself.
