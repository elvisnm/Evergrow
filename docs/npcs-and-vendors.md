...
Weapons / Armor / Accessories tabs filter the merchant’s 8-column tray. Shields belong to Armor; rings, amulets and caster foci belong to Accessories. Category counts show available pieces. Sold spaces stay empty until restock rather than shifting neighboring items. Buyback uses the same one-cell tray and its original sale prices. Actual inventory layout and dedicated charm rows stay intact, with an independent Auto-sort button. Every tray shrinks its cells to fit, allowing vertical scrolling but no horizontal overflow.
