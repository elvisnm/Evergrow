...
- **T:** open the skill atlas. Pan, zoom, search names/bonuses, highlight skill nodes by weapon or equipped gear, inspect a node, and allocate a connected node or an affordable complete route. Hovering or selecting a distant node previews the shortest route from the current build and its remaining point cost. Double-click commits the shortest affordable route; the native Allocate path button does the same. Right-click an invested node to refund one point for 25 gold (purchased ranks first). Disconnecting a chain opens a confirmation showing its nodes, ranks and total cost; alternate connected branches remain invested. Selection controls the sidebar; hover shows the node tooltip and route. Skill effects use aligned values with explicit units and duration, and the Preview accordion plays a compact demonstration in place. Canvas keyboard navigation follows neighboring stars.
...
Early active paths cost 2–6 points; advanced attacks cost 8–15; ultimates cost 23–33. Skills and tradeoffs are optional dead ends, never travel tolls. Twenty purchased ranks provide gradual damage and utility growth without mastery gates. Doctrines permit one paid choice per family with free reconfiguration. The atlas header and enchanter both provide a full skill respec for 25 gold per refunded node/rank point, with a confirmation and saved payment before commitment. The atlas also offers skill/weapon highlights and replayable skill and Technique previews. See [skill progression](skill-progression.md) for exact formulas, all choices, actions, save refunds and verification boundaries.
