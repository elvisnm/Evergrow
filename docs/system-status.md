...
Local September 19 atlas planning: Skills highlights and weapon/equipped-gear filters, quiet unallocated geometry with filled gold invested nodes and thicker gold paths retained through filters, a sidebar-edge open/close icon, a wider selection-driven sidebar with aligned effect values and expandable runtime skill/Technique previews, and a confirmed full respec at 25 gold per refunded node/rank point. Right-click refunds one point for 25 gold, reducing purchased ranks before removing the unlock. Disconnecting a chain opens a confirmation for the affected nodes/ranks and total cost; other connected branches remain invested. Atlas refunds persist the payment and refund before live commitment. No save migration/reset. See [skill progression](skill-progression.md).

Local September 20 atlas guide: a desktop-only nine-step first-open tour introduces node types, route planning, weapon requirements, inline previews, assignments/auras, ranks/Techniques, Doctrines/keystones and paid refunds. Players can skip, go back or replay from Guide (controller: right-stick click). Touch/mobile browsers and native Android omit the guide and replay control. The guide only changes presentation, restores the prior atlas view, and stores dismissal as a device preference. Character saves and builds remain unchanged; no reset.
