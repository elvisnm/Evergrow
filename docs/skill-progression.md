...
## Atlas planning controls — local September 19 follow-up

**Skills** highlights the 37 assignable unlocks (30 active skills and seven auras), dimming passive and Technique branches while keeping the route to the inspected skill bright. The weapon selector offers sword, axe, mace, dagger, bow, staff, wand, shield and **Equipped gear**. Family filters include unrestricted utility/aura skills; Equipped gear uses combat's actual hand compatibility, including an offhand wand and usable shields. Choosing a weapon enables Skills and updates highlights without changing the current map center or zoom; it also cancels any pending search fit. Recenter remains available explicitly. Toggling Skills updates highlights in place and cancels pending search fitting, preserving the current camera position and zoom. Turning Skills off restores All weapons. These controls intersect search. Clear resets every highlight filter. Selection, route allocation and point costs remain shared with the full atlas.

The atlas uses investment as its strongest visual state. Unallocated nodes have dim territory-tinted rims, dark lenses and thin muted connections, with no ambient node bloom. Allocated nodes have filled gold lenses, heavier rims and thicker luminous gold connections; these remain visible through search and weapon filters and on the mini-atlas. Unallocated route previews and focus use ice blue, while filter matches retain pink outer markers. Small overview nodes use a filled/hollow distinction as well as color. Background haze and dust stay subdued so the invested path reads first.

The inspector's open/close icon now sits at the upper-right map edge beside the sidebar, with a distinct open state, accessible label and tooltip. Selecting a node opens its details again. Clicking or tapping empty map space clears selection, its route and any playing preview, without panning or hiding the sidebar. Dragging continues to pan without clearing selection. Zoom controls contain only zoom, Origin and All.

Every skill and Technique has a collapsed **Preview** accordion, including locked choices. Expanding it plays a twelve-second demonstration in place beneath the effect values, using the real simulation, renderer and fixed CRT. Collapsing it disposes the demonstration. The compact stage provides Pause/Resume and Replay and has no popup or automatic loop. The source and output canvases match the displayed size at device density (up to 3×); fixed scene framing keeps the same action visible across densities. The preview uses sample compatible equipment and the selected active rank (rank 1 for locked skills); a Technique preview applies that Technique. Defensive skills receive incoming damage, empowerment skills demonstrate follow-up attacks, and piercing/surrounding actions stage appropriate target arrangements. The sidebar always follows selection; hovering only shows map tooltips. Ordinary panel refreshes preserve an open preview and its playback. Selecting a different node, changing the preview's rank/Technique, hiding the sidebar, opening respec or closing the atlas disposes its world and graphics. Backgrounding or scrolling the preview out of view suspends playback, while resizing retains its progress. It uses no playable character, saves, rewards or audio. `skill-showcase.ts` owns the shared disposable scene also used by the development playground and balance audits; production imports no development tool entrypoint.

The wider inspector uses one value format across active skills, auras, Techniques and rank comparisons: prominent damage, mana/reservation and cooldown values, followed by aligned effect/value rows. Units stay attached to numbers; durations and conditions appear below their value. Burn and ground fire each show their damage rate and duration. Damage percentages name their per-projectile/impact basis where applicable. Utility skills show their barrier, mitigation or empowerment instead of an empty damage value. The equipment requirement is a compact status line. Next-rank changes and Technique browsing use separate disclosures; numeric comparisons include effects removed by a Technique.

**Respec** is available in the atlas header as well as at the enchanter. Both charge 25 gold per refunded node/rank point. The atlas confirmation shows the complete refund, price and wallet balance before the action; an empty tree or insufficient gold disables confirmation. The reset clears nodes back to Origin, purchased/active ranks, Techniques, Overload, temporary skill/affix buffs, skill cooldowns and all five bindings. Attributes, equipment and world progress are retained. Health/mana only clamp to the resulting maxima; there is no refill. The atlas stages the payment/refund together and persists before live commitment; a failed save preserves the entire live build and wallet. Existing saves need no migration or automatic reset. Right-click an invested node to refund **one point for 25 gold**. A single-point refund remains immediate; a refund that disconnects other invested nodes opens a **Refund this chain?** confirmation. Purchased skill ranks are removed one at a time before the rank-one unlock; a lower active rank stays selected, capped to the remaining purchased rank. Removing an unlock clears its bindings and cooldown, while retained skills keep their cooldowns. The chain confirmation identifies the starting node, affected node count, purchased ranks, removed skills, total points returned and gold cost. Confirming refunds the starting node and every invested node that loses its connection to Origin, including their purchased ranks and Techniques. Alternate invested routes and unrelated branches are retained. Cancel/Escape changes nothing; insufficient gold disables confirmation. The command recomputes and checks the confirmed node set and point total before payment, rejecting a changed quote. Removing an active Technique returns that skill to Original; removing Overload turns it off. An aura rank refund is rejected if its increased reservation would consume all mana. Origin and unallocated nodes cannot be refunded. The same save-before-commit boundary protects single-point payments/refunds, and a tooltip shows the right-click cost. Successful refunds clear that node’s selection back to Origin without moving the camera, and suppress its hover highlight until the pointer leaves or the player explicitly selects it again. Failed refunds retain selection.

### First-open guide — local September 20 iteration

Opening the atlas for the first time on a desktop starts a nine-step, read-only guide. Touch/mobile browsers and native Android do not show the guide or its replay control; switching to a controller does not enable it there. It introduces the Root and point economy, travel/passive/notable nodes, skill and weapon filters, equipment requirements and an inline Fireball preview, assignments and aura reservation, ranks and Techniques, Doctrines and keystones, refunds, and navigation. The guide spotlights the relevant map node or control and uses the runtime node glyphs for examples. It never allocates points, changes equipment, assigns skills or spends gold.

Back and Next move through the steps. Skip, Escape or controller B dismiss the guide before closing the atlas; finishing or skipping records only a device preference. **Guide** in the header replays it at any time (controller: click the right stick). Blocked storage remembers dismissal for the current session. Replay temporarily stages the examples, then restores the player's selection, map position/zoom, filters, sidebar visibility and scroll position. An existing open preview restarts when returning. The existing save-free character review offers manual replay without automatically showing the guide or writing its preference.
...
A Doctrine family has three mutually exclusive nodes, connected to the same road. Buy one for one point. Once purchased, its detail panel offers free switching within that family; it replaces the owned choice, preserving node count, connectivity and the point ledger. Switching clears temporary affix/skill buffs, retains cooldowns and does not heal. This prevents keeping a temporary benefit from the previous choice. Full skill respec is available in the atlas and at the enchanter for 25 gold per refunded node/rank point; attributes are separate.
