import { difficultyLootWeights, worldDifficulty } from './world-difficulty.ts';
import { selectLootWeight } from './loot.ts';
...
export function eventRewards(site: EventRecord, playerLevel=site.level) {
  ...
  const items = Array.from({ length: count }, (_, i) => {
    ...
    const tier=selectLootWeight(table,random(10+i));
  ...
  return { items, gold:Math.round(gold*worldDifficulty(site.difficulty).gold), xp:Math.round(xp*worldDifficulty(site.difficulty).experience) };
}
